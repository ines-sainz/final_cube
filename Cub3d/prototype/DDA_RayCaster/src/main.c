...
float fix_ang(float a) //CAMBIADO
{
     if(a > 359)
        a -= 360;
     if(a < 0)
...
void draw_rays(t_player *player, t_game *game)
{
    ...
    while (r < 60)
    {
        ra = player->angle + ((float)r * rayStep) - (float)30; //CAMBIADO
        ...
        else//looking up or down. no hit
        {
            rx = player->x;
            ry = player->y;
            dof =  WIDTH / FOV; //CAMBIADO
        }

        while (dof < WIDTH) //CAMBIADO
        {
        ...
        if (lineH > HEIGHT)
            lineH = HEIGHT;
        draw_vertical_line(r, lineH, color, game); //CAMBIADO
