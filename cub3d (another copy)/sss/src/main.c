...
int touch(float rx, float ry, t_game *game) //thiis seems to not do anything
{
    int map_x, map_y;
...
void draw_rays(t_player *player, t_game *game)
{
    ...
    int     color;
    float   FOV = 60.0; //WAS COMENTED
    float   rayStep = FOV / WIDTH; //WAS COMENTED
...
/*void    draw_rays(t_player *player, t_game *game)
{
    double  directionx = cos(deg_to_rad(3.0 * PI / 2.0)); // angle faced
    double  directiony = sin(deg_to_rad(3.0 * PI / 2.0)); // angle faced
    ...
    while (i < WIDTH)
    {
        ...
        rayDirx = directionx + camarax * camaraX;
        rayDiry = directiony + camaray * camaraX;
        i++;
    }
}*/
