...
void    draw_texture(int wall_plane, float dist, float plane_x, int ray_iteration, t_game *game)
{
    int wall_height;
    int wall_y;
    int i;
    ...
    while (i < wall_height)
    {
        put_pixel(ray_iteration, wall_y, get_pixel_from_texture(wall_plane, plane_x, 64 * i / wall_height, game), game);
        wall_y++;
        i++;
    }
}
...
void    draw_a_ray(t_player *player, t_game *game, float ray_angle, int ray_iteration)
{
    ...
    prev_x = player->x;
    prev_y = player->y;
    ray_x = player->x + (delta_x * STEP);
    ray_y = player->y + (delta_y * STEP);
    ...
    {
        draw_texture(EAST, pitagoras(ray_x - player->x, ray_y - player->y)/* - pitagoras(1, cos(fabs(ray_angle - player->angle)) / sin(fabs(ray_angle - player->angle)))*/ , 64 - ((ray_y - (int)ray_y) * 64), ray_iteration, game);
    }
    else if ((int)prev_y > (int)ray_y)
    {
        draw_texture(SOUTH, pitagoras(ray_x - player->x, ray_y - player->y), (ray_x - (int)ray_x) * 64, ray_iteration, game);
    }
    else if ((int)prev_x < (int)ray_x)
    {
        draw_texture(WEST, pitagoras(ray_x - player->x, ray_y - player->y), (ray_y - (int)ray_y) * 64, ray_iteration, game);
    }
    else
    {
        draw_texture(NORTH, pitagoras(ray_x - player->x, ray_y - player->y), 64 - ((ray_x - (int)ray_x) * 64), ray_iteration, game);
    }
}
